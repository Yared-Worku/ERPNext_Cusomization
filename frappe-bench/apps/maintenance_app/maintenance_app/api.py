...
@frappe.whitelist(allow_guest=True)
def get_service_tickets_orm():
    """
    [READ] Fetches Service Tickets and associated Spare Parts using Frappe ORM.
    """
    tickets = frappe.get_all(
        "Service Ticket",
        fields=["name", "equipment_name", "status", "labor_fee", "total_cost"]
    )

    for ticket in tickets:
        ticket["spare_parts"] = frappe.get_all(
            "Service Spare Part",
            filters={"parent": ticket["name"]},
            fields=["part_name", "qty", "unit_price", "amount"]
        )
...
@frappe.whitelist(allow_guest=True)
def create_service_ticket_orm(equipment_name: str, labor_fee: float = 0.0, spare_parts: list | str | None = None):
    """
    [CREATE] Creates a Service Ticket document with child rows using frappe.get_doc().
    """
    if isinstance(spare_parts, str):
        spare_parts = json.loads(spare_parts)
    elif spare_parts is None:
    ...
    doc.total_cost = labor_fee + total_parts_cost
    ...
    return {
        "status": "success",
        "message": "Service Ticket created successfully using ORM",
        "ticket_id": doc.name,
...
@frappe.whitelist(allow_guest=True)
def update_ticket_status_orm(ticket_id: str, status: str):
    """
    [UPDATE] Updates document fields and saves using doc.save().
    """
    if not frappe.db.exists("Service Ticket", ticket_id):
        return {"status": "error", "message": f"Ticket {ticket_id} not found."}

    doc = frappe.get_doc("Service Ticket", ticket_id)
    doc.status = status
...
@frappe.whitelist(allow_guest=True)
def delete_service_ticket_orm(ticket_id: str):
    """
    [DELETE] Removes parent document and automatically cascades child row deletion.
    """
    if not frappe.db.exists("Service Ticket", ticket_id):
        return {"status": "error", "message": f"Ticket {ticket_id} not found."}

    # Automatically handles parent + child records deletion in a single atomic transaction
    frappe.delete_doc("Service Ticket", ticket_id, ignore_permissions=True)

    return {
        "status": "success",
        "message": f"Ticket {ticket_id} and all child rows deleted successfully via ORM",
        "deleted_ticket_id": ticket_id
    }
